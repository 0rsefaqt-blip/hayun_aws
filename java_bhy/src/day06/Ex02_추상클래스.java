...
public class Ex02_추상클래스 {
	...
	public static void main(String[] args) {
		/* 추상 클래스 p.290
		 * - 클래스 앞에 abstract 앱스트랙트 키워드가 붙은 클래스
		 ...
		 * 
		 * 추상 메서드 p.290 틀만 잡아놓음
		 * - 메서드를 정의하는 부분만 있고, 구현하는 부분이 없는 메서드
		 * - 이런 기능이 있을 예정인데, 아직 구현 안했음
		 * - 일반 메서드 형태
		 *   리턴타입 메서드명(매개변수) { 구현 }
		 * - 추상 메서드 형태
		 *   리턴타입 메서드명(매개변수);          (구현을 안해서 세미콜론으로 끝냄)
		 * 
		 */
//		Computer com = new Computer(); 안됨
...
class LgComputer extends Computer{
	...
	@Override //추상메서드를만들거나 추상클래스로 만들어야됨 이건 추상메서드만들기한것
