...
public class Ex01_다형성 {
	...
	public static void main(String[] args) {
		...
		AirConditioner [] aircons = new AirConditioner[10]; //기존방식
		Tv [] Tvs = new Tv[10];
		int acCount = 0; //에어컨 개수
		int tvCount = 0; //티비 대수 
		
		AirConditioner ac1 = new AirConditioner();//공장에서 에어컨 만듦
		AirConditioner ac2 = new AirConditioner();//공장에서 에어컨 만듦
		aircons[acCount] = ac1;//매장에 에어컨 추가
		acCount++;             //매장에 있는 에어컨 대수 1대 증가
		aircons[acCount] = ac2;//매장에 에어컨 추가
		acCount++;             //매장에 있는 에어컨 대수 1대 증가
		
		
		//다형성을 이용하면 한 종류를 이용하여 여러종류의 클래스를 관리할 수 있다
		Electronics [] electronics = new Electronics[20];//다형성 방식
		int eCount = 0;//전자제품수
		electronics[eCount] = ac1; //업캐스팅
		eCount++;
