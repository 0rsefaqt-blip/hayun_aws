...
UPDATE trainer AS t
        JOIN
    trainer AS leader on leader.name = '김철수'
SET 
...
# 11~15번 : 재귀 공통 테이블식(Recursive CTE)
...
# 새로운 강좌를 등록
# - 강좌명 : 크로스핏 초급반, 정원 : 30명, 수강료 : 50,000원 담당 : 김철수(1)
insert into program(title, personal, fee, trainer_id) 
values('모닝 요가', 20, 100000, 2), ('지옥의 크로스핏', 15, 150000, 3), 
('크로스핏 초급반', 30, 50000, 1);

# 수강 신청 
# - 이순신(1) 회원이 모닝 요가(1)와 지옥의 크로스핏(2)을 수강 신청 후 결제 완료 함
# - 성춘향(2) 회원이 모닝 요가(1)를 수강 신청 함 
# - 결재완료하면 '결제완료', 수강신청만 하면 '신청'으로 관리
