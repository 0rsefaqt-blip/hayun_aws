...
# 컬럼 not in(값1,값2, ..., 값N) : 컬럼 값이 in()안에 있는 값들과 다르면 참
...
select * from student where grade in(1,2); #위와 같은 의미

# 등록된 성적을 조회
select * from score;
# 성적이 60이상 100이하를 조회
select * from score where score >= 90 and score <= 100;
select * from score where score between 90 and 100; #위와 같은 의미

# 성이 홍씨인 학생을 조회
select * from student where name like '홍%';

# 이름이 3글자인 학생을 조회
select * from student where name like '___'; #언더바3개

# 정렬
# select distinct 컬럼1, 컬럼2, ..., 컬럼N
# from 테이블명
# [where 조건]
# [order by 컬럼1 [asc|desc] [,컬럼2 [asc|desc]]] 정렬 
...
# 학생 성적을 처음 2개만 조회
select * from score limit 0, 2; # limit (page -1) * 개수, 개수;
# 학생 성적을 2번째 2개만 조회
...
# from 테이블1 
# join 테이블2 
# on 테이블1.컬럼 = 테이블2.컬럼 //1줄을 4줄에 나눠서 씀

# 테이블1이 테이블2를 참조하는 경우 테이블1.컬럼은 외래키, 테이블2.컬럼은 기본키
# 각 컬럼명이 다른 테이블 컬럼들과 중복되지 않으면 앞에 테이블명을 제거해도 됨
	# on 컬럼1 = 컬럼2 # 컬럼1이 중복안됨, 컬럼2가 중복안됨
# 컬럼명이 같은 경우 
	# on 테이블1.컬럼 = 테이블2.컬럼 
    # 대신 
    # using(컬럼)을 사용
...
FROM
    score
        JOIN
    student ON student.code = score.st_code
GROUP BY st_code;

# 2. outer join
# - 기준 테이블을 기준으로 합침 
	# 기준 테이블 옆에 다른 테이블을 가져와서 값이 있는 얘들은 붙이고 없으면 null을 채움 
# - 제품별 판매액을 조회할 때 사용 
#	=> 판매 안된 제품은 0원으로 조회할 때 사용 #이너조인은 판매안된제품은조회안됨 얜 0원으로 조회
...
# 서브쿼리 사용 위치 
	# select문에서 컬럼 대신 사용
    # from에서 테이블 대신 사용 : 이 때 as 테이블 별칭 추가해야함 
    # where절에서 특정 값 대신 사용
