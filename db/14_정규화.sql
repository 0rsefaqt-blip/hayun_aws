...
# 제2정규화 
    ...
    # 기본키가 2개 이상으로 이루어져 있는데, 어떤 정보가 그중 하나만 알아도 결정된다면 따로 빼자.
    
# 제3정규화
	# 이행함수 종속 제거 
    # 기본키가 아닌 컬럼들끼리 종속 관계에 있으면 분리 
    # 기본키가 아닌 다른 정보가 또 다른 정보를 결정하고 있네?, 따로 빼자.
    
# 2정규화 = "키의 일부 때문에 생긴 문제"
# 3정규화 = "키가 아닌 놈 때문에 생긴 문제"
    
# BCNF 
	# 복합키 내에서 종속성 제거 

# enrollment_unf 테이블은 정규화되지 않은 테이블. 기본키는 student_id, course_id 
use nf;
...
# student_name과 phone_number가 student_id에 종속됨 => 제2정규화 
