...
# 			테이블의 컬럼 순서와 insert 할 때 컬럼 순서는 달라도 됨
# - 문법2
# 	insert into 테이블명
# 	values(값1, 값2, ..., 값N);
# 	- 설명 : 값들은 테이블에 있는 컬럼 수만큼 써야 하고, 컬럼 순서대로 값 지정

# 컴퓨터공학과, 학과 코드는 160, 주소 1캠퍼스 A관 401호, 학과장 미정
insert into major(code, address, name, boss_id)
values(160, 'A관 401호', '컴퓨터공학과', null); 

# 유아교육과, 학과 코드는 456, 주소 4캠퍼스 B관 102호, 학과장 미정
insert into major
...
# - 안전모드 끄는 방법
# Edit > Preferences > SQL Editer > Safe Updates 체크박스 체크 해제
delete from sample.major where address = 'A관 401호'; #(sample.)을 붙이면 db를 선택하지않아도 실행
